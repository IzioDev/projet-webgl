...
export class Model {
    ...
    handleLoadedObject (objData) {
        ...
        gl.vertexAttribPointer(1, (this.normalBuffer as any).itemSize, gl.FLOAT, false, 0, 0);
    ...
    sendUniformVariables() {
        ...
        if(this.loaded) {
            ...
            gl.uniformMatrix4fv((shader as any).modelMatrixUniform,false,this.modelMatrix);
            gl.uniformMatrix4fv((shader as any).viewMatrixUniform,false,this.viewMatrix);
            gl.uniformMatrix4fv((shader as any).projMatrixUniform,false,this.projMatrix);

            //couleur du model
            gl.uniform3f((shader as any).kdUniform, this.col[0], this.col[1], this.col[2]);

            //light
            gl.uniform3f((shader as any).lightUniform, this.light[0], this.light[1], this.light[2]);
